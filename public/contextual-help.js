...
(function () {
  ...
  const CFG = {
    appSlug: 'neocohmetrix',                 // namespace for locationId (must == server APP_SLUG)
    langCookie: 'ncm_i18n_lang',             // the SPA writes the UI language code to this cookie
    api: {
      config:     '/api/config/help-hover',  // dwell ms + auto-open flag
      ask:        '/api/help/ask',           // main Q&A endpoint (LLM gateway, public)
      tts:        '',                         // TTS not wired — Read-aloud chip stays hidden
      languages:  '/api/i18n/languages',     // optional — drives tts coverage gate (unused while tts off)
    },
    feedbackUrl: null,                       // app has its own feedback flow; hide the button here
...
#cx-help-fb-overlay iframe{width:100%;height:100%;border:0;display:block;background:#fff;}`;
  ...
  function findMeaningfulContainer(el) {
    ...
    while (cur && cur !== document.body && hops < 8) {
      // App-specific interest selectors — tuned for this vanilla-JS SPA's DOM.
      const interesting = cur.matches && cur.matches(
        ...
        '.nav-item, .form-row, .form-row-grid, .section-title, .modal-title, ' +
        '.topbar-title, .stat-card, .layer-item, .metric-row, .help-def-card, ' +
  ...
  function updateToggleLabel() {
    ...
    if (!btn) return;
    // Wrap in data-i18n so the label localizes via the host i18n sweep (not t(),
    // which is identity here — see i18n-help-bridge.js echo-of-source guard).
    if (state.userEnabled) { btn.innerHTML = '<span data-i18n>Auto-help: on</span>';  btn.classList.add('on'); }
    else                   { btn.innerHTML = '<span data-i18n>Auto-help: off</span>'; btn.classList.remove('on'); }
    if (window.I18n && I18n.applyTo) I18n.applyTo(btn);
  }
  ...
  function renderContextChip() {
    ...
    // The "Context" label + "This area" fallback are static chrome -> data-i18n so
    // they localize. The value is live page text (headings/view) and is left as-is.
    const valHTML = bits.length
      ? '<span class="cxh-context-val">' + escapeHTML(bits.join(' · ')) + '</span>'
      : '<span class="cxh-context-val" data-i18n>This area</span>';
    el.innerHTML = '<span class="cxh-context-label"><span data-i18n>Context</span>:</span> ' + valHTML;
