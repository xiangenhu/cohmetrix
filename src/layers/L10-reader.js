...
const LAYER_ID = 'L10';
const LAYER_NAME = 'Reader-Adaptive';
...
const POPULATION_NORMS = {
  ...
  'L7.2': { mean: 1.8, sd: 0.7 },   // premises per claim
};
...
async function analyze(doc, { layerResults, learnerProfile } = {}) {
  ...
  const l7Score = layerResults?.L7?.rawValues?.premise_to_claim_ratio ?? 1.5;
  ...
  const zArgument = (l7Score - expectedPremises) / POPULATION_NORMS['L7.2'].sd;
...
Text excerpt: ${doc.text.substring(0, 1500)}`);
  ...
  const metrics = {
    'L10.1': { value: round(zLexical, 1), unit: 'z', label: 'Lexical challenge (z)' },
    'L10.2': { value: round(zSyntactic, 1), unit: 'z', label: 'Syntactic challenge (z)' },
    'L10.3': { value: round(zSemantic, 1), unit: 'z', label: 'Semantic gap score (z)' },
    'L10.4': { value: round(zArgument, 1), unit: 'z', label: 'Argumentation readiness (z)' },
    'L10.5': { value: round(globalDifficulty, 1), unit: 'z', label: 'Global difficulty estimate' },
    'L10.6': { value: round(zpdProximity, 2), unit: 'ZPD', label: 'ZPD proximity index' },
    'L10.7': { value: writingAboveLevel, unit: 'flag', label: 'Writing-above-level indicator' },
    'L10.8': { value: scaffoldType, unit: 'type', label: 'Suggested scaffold type' },
  };
